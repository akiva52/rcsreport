...
const PPT_IMG_RE = /\.(png|jpe?g|gif|webp)$/i;
const parseXml = str => {
  const doc = new DOMParser().parseFromString(str, "application/xml");
  return doc.getElementsByTagName("parsererror").length ? null : doc;
};
const zipXml = async (zip, path) => (zip.files[path] ? parseXml(await zip.files[path].async("string")) : null);
const els = (root, ns, tag) => (root ? Array.from(root.getElementsByTagNameNS(OOXML_NS[ns], tag)) : []);
const emu = (el, attr) => { const n = parseInt(el?.getAttribute(attr) ?? "", 10); return Number.isFinite(n) ? n : 0; };
// resolve a relationship Target ("../media/image1.png") against its part's folder ("ppt/slides/")
const resolvePart = (baseDir, target) => {
...
const relMap = (relsDoc, baseDir) => {
  const map = {};
  for (const rel of els(relsDoc, "pkg", "Relationship")) {
    if (rel.getAttribute("TargetMode") === "External") continue;
    const id = rel.getAttribute("Id"), target = rel.getAttribute("Target");
    if (id && target) map[id] = resolvePart(baseDir, target);
  }
  return map;
};
// pictures on a slide: resolved media path + y-position/height (EMU, 0 when absent)
const slidePics = (slideDoc, rels) => els(slideDoc, "p", "pic").map(pic => {
  const blip = els(pic, "a", "blip")[0];
  const path = blip && rels[blip.getAttributeNS(OOXML_NS.r, "embed")];
  if (!path || !PPT_IMG_RE.test(path)) return null;
  const off = els(pic, "a", "off")[0], ext = els(pic, "a", "ext")[0];
  return { path, x: emu(off, "x"), y: emu(off, "y"), cx: emu(ext, "cx"), cy: emu(ext, "cy") };
}).filter(Boolean);
// text blocks on a slide: one entry per shape with a <p:txBody>, paragraphs joined with a space
const slideTexts = (slideDoc) => els(slideDoc, "p", "txBody").map(body => {
  const text = els(body, "a", "p")
    .map(par => els(par, "a", "t").map(t => t.textContent).join(""))
    .map(s => s.replace(/\s+/g, " ").trim())
    .filter(Boolean)
    .join(" ");
  if (!text) return null;
  const shape = body.parentNode;
  const off = els(shape, "a", "off")[0], ext = els(shape, "a", "ext")[0];
  return { text, x: emu(off, "x"), y: emu(off, "y"), cx: emu(ext, "cx") };
}).filter(Boolean);
// pair each text block with the picture whose bottom edge is nearest above it; pictures whose
// bottoms line up within TIE_EMU count as one row, so side-by-side photos keep their own captions
const TIE_EMU = 228600; // 0.25"
const pairCaptions = (pics, texts) => {
  const buckets = pics.map(() => []);
  if (!pics.length) return buckets;
  for (const t of texts) {
    let best = 0;
    if (pics.length > 1) {
      const above = pics.map((p, i) => ({ i, bottom: p.y + p.cy })).filter(p => p.bottom <= t.y);
      if (above.length) {
        const lowest = Math.max(...above.map(p => p.bottom));
        const row = above.filter(p => lowest - p.bottom <= TIE_EMU);
        const mid = t.x + t.cx / 2;
        const dist = i => Math.abs(pics[i].x + pics[i].cx / 2 - mid);
        best = row.reduce((a, b) => (dist(a.i) <= dist(b.i) ? a : b)).i;
      }
    }
    buckets[best].push(t.text);
  }
  return buckets;
};

// photo pages: a captioned entry owns a page, runs of uncaptioned entries pair up (mirrors the PDF)
const photoPageCount = entries => {
  let pages = 0, slotsLeft = 0;
  for (const e of entries) {
    if (e.caption) { pages++; slotsLeft = 0; }
    else { if (slotsLeft === 0) { pages++; slotsLeft = 2; } slotsLeft--; }
  }
  return pages;
};
...
function App() {
  ...
  const handlePPT = useCallback(async e => {
    ...
    try {
      ...
      // slide order from ppt/presentation.xml <p:sldIdLst>, falling back to numeric filename sort
      ...
      let slidePaths = [];
      try {
        const presRels = relMap(await zipXml(zip, "ppt/_rels/presentation.xml.rels"), "ppt/");
        const sldIdLst = els(await zipXml(zip, "ppt/presentation.xml"), "p", "sldIdLst")[0];
        slidePaths = els(sldIdLst, "p", "sldId")
          .map(n => presRels[n.getAttributeNS(OOXML_NS.r, "id")])
          .filter(path => path && zip.files[path]);
      } catch (err) { slidePaths = []; }
      if (!slidePaths.length) slidePaths = byFileName();

      const slides = [];
      for (const slidePath of slidePaths) {
        const baseDir = slidePath.slice(0, slidePath.lastIndexOf("/") + 1);
        const relsPath = `${baseDir}_rels/${slidePath.slice(baseDir.length)}.rels`;
        const slideDoc = await zipXml(zip, slidePath);
        if (!slideDoc) continue;
        const rels = relMap(await zipXml(zip, relsPath), baseDir);
        const pics = slidePics(slideDoc, rels).filter(pic => zip.files[pic.path]).sort((a, b) => a.y - b.y);
        if (!pics.length) continue;
        const texts = slideTexts(slideDoc).sort((a, b) => a.y - b.y);
        const captions = pairCaptions(pics, texts);
        for (let i = 0; i < pics.length; i++) {
          const blob = await zip.files[pics[i].path].async("blob");
          slides.push({
            name: pics[i].path.split("/").pop(),
            dataUrl: await readAsDataURL(blob),
            caption: captions[i].join(" ") || null,
          });
        }
      }
      ...
      if (slides.length === 0) {
        ...
        for (const p of media) {
          ...
          slides.push({ name: p.split("/").pop(), dataUrl: await readAsDataURL(blob), caption: null });
        }
      }
      slidesRef.current = slides;
      setPptSlides(slides.map(s => ({ name: s.name, caption: s.caption })));
                  ...
                  : <div style={{ padding:"12px 16px", border:"0.5px solid var(--color-border-tertiary)", borderRadius:10, background:"var(--color-background-secondary)" }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          ...
                          <div><p style={{ fontSize:13, fontWeight:500, margin:0 }}>{pptFileName}</p><p style={{ fontSize:11, color:"var(--color-text-tertiary)", margin:"2px 0 0" }}>{pptSlides.length} photos · {photoPageCount(pptSlides)} pages</p></div>
                        </div>
